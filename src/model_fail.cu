...
//pipelining for num layers
cudaStream_t streams_extra[NGPU][NUM_BATCH];
...
//now Each GPU has the num layer's activations
ActivationGPU **embd_a[NGPU], **ffn_proj_a[NGPU];
ActivationGPU **mha_qkv_proj_a[NGPU], **mha_out_a[NGPU], *mha_mask_a[NGPU], **mha_q_a[NGPU], **mha_k_a[NGPU], **mha_v_a[NGPU],
...
void alloc_activations_gpu(size_t batch_size, size_t seq_len){
  for(int gpu_id = 0; gpu_id < ngpu; gpu_id++){
    ...
    embd_a[gpu_id] = new ActivationGPU*[NUM_BATCH];
    ffn_proj_a[gpu_id] = new ActivationGPU*[NUM_BATCH];
    mha_qkv_proj_a[gpu_id] = new ActivationGPU*[NUM_BATCH];
    mha_out_a[gpu_id] = new ActivationGPU*[NUM_BATCH];
    mha_mask_a[gpu_id] = new ActivationGPU({seq_len, seq_len});
    
    mha_q_a[gpu_id] = new ActivationGPU*[NUM_BATCH];
    mha_k_a[gpu_id] = new ActivationGPU*[NUM_BATCH];
    mha_v_a[gpu_id] = new ActivationGPU*[NUM_BATCH];
    mha_attn_out_a[gpu_id] = new ActivationGPU*[NUM_BATCH];
    mha_concat_head_a[gpu_id] = new ActivationGPU*[NUM_BATCH];
    attn_score_a[gpu_id] = new ActivationGPU*[NUM_BATCH];
    k_transposed_a[gpu_id] = new ActivationGPU*[NUM_BATCH];
    wte_transposed_a[gpu_id] = new ActivationGPU({HIDDEN_DIM, NUM_VOCAB});

    

    residual_a[gpu_id] = new ActivationGPU*[NUM_BATCH];
    logit_a[gpu_id] = new ActivationGPU*[NUM_BATCH];
    transformer_block_a[gpu_id] = new ActivationGPU*[NUM_BATCH];

    next_token_id[gpu_id] = new int*[NUM_BATCH];


    for(int num_batch = 0; num_batch < NUM_BATCH; num_batch++){
      embd_a[gpu_id][num_batch] = new ActivationGPU({batch_size, seq_len, HIDDEN_DIM});
      ffn_proj_a[gpu_id][num_batch] = new ActivationGPU({batch_size, seq_len, 4 * HIDDEN_DIM});
      mha_qkv_proj_a[gpu_id][num_batch] = new ActivationGPU({batch_size, seq_len, 3 * HIDDEN_DIM});
      mha_out_a[gpu_id][num_batch] = new ActivationGPU({batch_size, seq_len, HIDDEN_DIM});
      
      mha_q_a[gpu_id][num_batch] = new ActivationGPU({NUM_HEAD, batch_size, seq_len, HIDDEN_DIM / NUM_HEAD});
      mha_k_a[gpu_id][num_batch] = new ActivationGPU({NUM_HEAD, batch_size, seq_len, HIDDEN_DIM / NUM_HEAD});
      mha_v_a[gpu_id][num_batch] = new ActivationGPU({NUM_HEAD, batch_size, seq_len, HIDDEN_DIM / NUM_HEAD});
      mha_attn_out_a[gpu_id][num_batch] = new ActivationGPU({NUM_HEAD, batch_size, seq_len, HIDDEN_DIM / NUM_HEAD});
      mha_concat_head_a[gpu_id][num_batch] = new ActivationGPU({batch_size, seq_len, HIDDEN_DIM});
      
      attn_score_a[gpu_id][num_batch] = new ActivationGPU({NUM_HEAD, batch_size, seq_len, seq_len});
      k_transposed_a[gpu_id][num_batch] = new ActivationGPU({NUM_HEAD, batch_size, HIDDEN_DIM / NUM_HEAD, seq_len});
      
      residual_a[gpu_id][num_batch] = new ActivationGPU({batch_size, seq_len, HIDDEN_DIM});
      logit_a[gpu_id][num_batch] = new ActivationGPU({batch_size, seq_len, NUM_VOCAB});
      transformer_block_a[gpu_id][num_batch] = new ActivationGPU({batch_size, seq_len, HIDDEN_DIM});
      CHECK_CUDA(cudaMalloc(&next_token_id[gpu_id][num_batch], batch_size * sizeof(int)));
    }
  }
  // //generate the positional mask
  // for(int i = 0; i < ngpu; i++){
  //   CHECK_CUDA(cudaSetDevice(i));
  //   generate_mask_gpu(mha_mask_a[i], streams_extra[i][0]);
  //   transpose_no_group_gpu(wte_gpu[i], wte_transposed_a[i], NUM_VOCAB, HIDDEN_DIM, streams_extra[i][1]);
  // }
}
...
void ffn_gpu(Activation *in, Parameter *mlp1_w, Parameter *mlp1_b,
         Parameter *mlp2_w, Parameter *mlp2_b, Activation *out, size_t batch_size, size_t seq_len, size_t hidden_dim, cudaStream_t stream, int gpu_id, int num_batch){
  ...
  linear_gpu(in, mlp1_w, mlp1_b, ffn_proj_a[gpu_id][num_batch], batch_size, seq_len, stream);
  //GELU Kernel
  gelu_gpu(ffn_proj_a[gpu_id][num_batch], batch_size* seq_len * 4 * hidden_dim, stream);
  //Projection Down Kernel
  linear_gpu(ffn_proj_a[gpu_id][num_batch], mlp2_w, mlp2_b, out, batch_size, seq_len, stream);
}
...
void attention_gpu(Activation *q, Activation *k, Activation *v, Activation *mask,
               Activation *out, size_t batch_size, size_t seq_len, size_t total_seq_len , cudaStream_t stream, int gpu_id, int num_batch) {
  //The same process as the CPU version, but we need to add the stream parameter
  //It should be executed in the gpu, it shouldn't be communicated with the CPU
  //So we need to add the stream parameter and invoke the multiple sequential kernels in the GPU
  //Get Attention score by q @ k Kernel
  transpose_gpu(k, k_transposed_a[gpu_id][num_batch], NUM_HEAD*batch_size, seq_len, HIDDEN_DIM/NUM_HEAD, stream);
  matmul_gpu(q, k_transposed_a[gpu_id][num_batch], attn_score_a[gpu_id][num_batch], NUM_HEAD*batch_size, seq_len, HIDDEN_DIM/NUM_HEAD, seq_len ,stream);
  //Scaling Kernel
  scaling_gpu(attn_score_a[gpu_id][num_batch], (1.0 / sqrt(k->shape[3])), stream);
  //Masking Kernel
  add_mask(attn_score_a[gpu_id][num_batch], mask, NUM_HEAD*batch_size, seq_len, total_seq_len ,stream);
  //Softmax Kernel
  softmax_gpu(attn_score_a[gpu_id][num_batch], NUM_HEAD*batch_size, seq_len, seq_len, stream);
  //Attention score @ v Kernel
  matmul_gpu(attn_score_a[gpu_id][num_batch], v, out, NUM_HEAD*batch_size, seq_len, seq_len ,HIDDEN_DIM / NUM_HEAD, stream);
}
...
void mha_gpu(Activation *in, Parameter *attn_b, Parameter *attn_w,
         Parameter *proj_b, Parameter *proj_w, Activation *out, size_t batch_size, size_t seq_len, size_t total_seq_len, cudaStream_t stream, int gpu_id, int num_batch) {
  //The same process as the CPU version, but we need to add the stream parameter
  //It should be executed in the gpu, it shouldn't be communicated with the CPU
  //So we need to add the stream parameter and invoke the multiple sequential kernels in the GPU
  //QKV projection Kernel
  linear_gpu(in, attn_w, attn_b, mha_qkv_proj_a[gpu_id][num_batch], batch_size, seq_len ,stream);
  //Generate mask to hide future inputs Kernel
  //generate_mask_gpu(mha_mask_a[gpu_id], stream);
  split_qkv_head_gpu(mha_qkv_proj_a[gpu_id][num_batch], mha_q_a[gpu_id][num_batch], mha_k_a[gpu_id][num_batch], mha_v_a[gpu_id][num_batch], NUM_HEAD, batch_size*seq_len, HIDDEN_DIM, stream);
  attention_gpu(mha_q_a[gpu_id][num_batch], mha_k_a[gpu_id][num_batch], mha_v_a[gpu_id][num_batch], mha_mask_a[gpu_id], mha_attn_out_a[gpu_id][num_batch], batch_size, seq_len, total_seq_len, stream, gpu_id, num_batch);
  //Concat each heads Kernel
  concat_head_gpu(mha_attn_out_a[gpu_id][num_batch], mha_concat_head_a[gpu_id][num_batch], NUM_HEAD ,batch_size, seq_len, HIDDEN_DIM, stream);
  //OUT projection Kernel
  linear_gpu(mha_concat_head_a[gpu_id][num_batch], proj_w, proj_b, out, batch_size, seq_len, stream);
}
...
void transformer_block_gpu(ActivationGPU *in, ParameterGPU *attn_b, ParameterGPU *attn_w,
                       ...
                       ParameterGPU *mlp2_w, ActivationGPU *out, size_t batch_size, size_t seq_len, size_t total_seq_len, cudaStream_t stream, int gpu_id, int num_batch) {
  //The same process as the CPU version, but we need to add the stream parameter
  //It should be executed in the gpu, it shouldn't be communicated with the CPU
  //So we need to add the stream parameter and invoke the multiple sequential kernels in the GPU
  //Copy Residual Kernel
  //Do copy with extra stream.
  copy_gpu(in, residual_a[gpu_id][num_batch], stream);
  //Layer Normalization Kernel
  layer_norm_gpu(in, ln_1_g, ln_1_b, batch_size, seq_len, stream);
  //Masked Multi-Head Self-Attention Kernel
  mha_gpu(in, attn_b, attn_w, proj_b, proj_w, mha_out_a[gpu_id][num_batch], batch_size, seq_len, total_seq_len, stream, gpu_id, num_batch);
  add_copy_gpu(mha_out_a[gpu_id][num_batch], residual_a[gpu_id][num_batch], batch_size, seq_len, HIDDEN_DIM, stream);
  //Layer Normalization Kernel
  layer_norm_gpu(mha_out_a[gpu_id][num_batch], ln_2_g, ln_2_b, batch_size, seq_len, stream);
  //Position-wise Feed-Forward Network Kernel
  ffn_gpu(mha_out_a[gpu_id][num_batch], mlp1_w, mlp1_b, mlp2_w, mlp2_b, out, batch_size, seq_len, HIDDEN_DIM, stream, gpu_id, num_batch);
  //Add Residual Kernel
  add_gpu(out, residual_a[gpu_id][num_batch], batch_size, seq_len, HIDDEN_DIM, stream);
}
...
void generate_tokens(int *input, int *output, size_t n_prompt, size_t n_token) {
  ...
  int prompts_per_gpu = prompts_per_rank / ngpu;//batch_size * 12
  //int batch_size = prompts_per_gpu;
  //prompts_per_gpu = 512*12 = 6144, 8192*12 = 98304 is our final input_prompts
  int batch_size = prompts_per_gpu/NUM_BATCH;
  ...
  for(int i = 0; i < ngpu; i++){
    CHECK_CUDA(cudaSetDevice(i));
    //allocate the gpu_id
    gpu_input[i] = new int*[NUM_BATCH];
    gpu_output[i] = new int*[NUM_BATCH];
    
    for(int num_batch = 0; num_batch < NUM_BATCH; num_batch++){
      CHECK_CUDA(cudaStreamCreate(&streams_extra[i][num_batch]));
      CHECK_CUDA(cudaMalloc(&gpu_input[i][num_batch], batch_size * input_seq_len * sizeof(int)));
      CHECK_CUDA(cudaMalloc(&gpu_output[i][num_batch], batch_size * output_seq_len * sizeof(int)));
      CHECK_CUDA(cudaMemcpyAsync(gpu_input[i][num_batch], local_input + i * prompts_per_gpu * input_seq_len + num_batch * batch_size * input_seq_len, batch_size * input_seq_len * sizeof(int), cudaMemcpyHostToDevice, streams_extra[i][num_batch]));
    }
  }
  ...
  for(int i = 0; i < ngpu; i++){
    CHECK_CUDA(cudaSetDevice(i));
    generate_mask_gpu(mha_mask_a[i], streams_extra[i][0]);
    transpose_no_group_gpu(wte_gpu[i], wte_transposed_a[i], NUM_VOCAB, HIDDEN_DIM, streams_extra[i][0]);
  }
  ...
  for(int t = 0; t < output_seq_len; t++){
    ...
    for(int i =0; i <ngpu; i++){
      ...
      for(int num_batch = 0; num_batch < NUM_BATCH; num_batch++){
        token_pos_embedding_gpu(gpu_input[i][num_batch], gpu_output[i][num_batch], wte_gpu[i], wpe_gpu[i], embd_a[i][num_batch], batch_size, input_seq_len, output_seq_len, t, streams_extra[i][num_batch]);
        //record the event that num_batch has passed the token_pos_embedding
        //CHECK_CUDA(cudaEventRecord(events_extra[i][num_batch], streams_extra[i][num_batch]));
      }
    }
    //Forward path of Transformer blocks
    for(int i = 0; i < ngpu; i++){
      CHECK_CUDA(cudaSetDevice(i));
      //There are num_batch x num_layer events for gpu i
      //events[i][j][k] means that for gpu i, jth batch has passed kth layer
      for (size_t l = 0; l < NUM_LAYER; l++) {
        for(size_t num_batch = 0; num_batch < NUM_BATCH; num_batch++){
          transformer_block_gpu(embd_a[i][num_batch], attn_b_gpu[i][l], attn_w_gpu[i][l], proj_b_gpu[i][l], proj_w_gpu[i][l],
                                ln_1_b_gpu[i][l], ln_1_g_gpu[i][l], ln_2_b_gpu[i][l], ln_2_g_gpu[i][l],
                                mlp1_b_gpu[i][l], mlp1_w_gpu[i][l], mlp2_b_gpu[i][l], mlp2_w_gpu[i][l],
                                transformer_block_a[i][num_batch], batch_size, input_seq_len + t, total_seq_len ,streams_extra[i][num_batch], i, num_batch);
          /* Copy output to embd_a for next block */
          copy_gpu(transformer_block_a[i][num_batch], embd_a[i][num_batch], streams_extra[i][num_batch]);
        }
      }
    }
    ...
    for(int i = 0; i < ngpu; i++){
      ...
      for(int num_batch = 0; num_batch < NUM_BATCH; num_batch++){
        ...
        layer_norm_gpu(embd_a[i][num_batch], ln_f_g_gpu[i], ln_f_b_gpu[i], batch_size, input_seq_len + t, streams_extra[i][num_batch]);
        //Final Matmul for the final layer
        final_matmul(embd_a[i][num_batch], wte_transposed_a[i], logit_a[i][num_batch], batch_size*(input_seq_len + t),  HIDDEN_DIM, NUM_VOCAB, streams_extra[i][num_batch]);
      }
    }
    ...
    for(int i = 0; i < ngpu; i++){
      CHECK_CUDA(cudaSetDevice(i));
      for(int num_batch = 0; num_batch < NUM_BATCH; num_batch++){
        top1_sampling_gpu(logit_a[i][num_batch], batch_size, input_seq_len, t, gpu_output[i][num_batch], output_seq_len, streams_extra[i][num_batch]);
      }
    }
  }
  ...
  for(int i = 0; i < ngpu; i++){
    CHECK_CUDA(cudaSetDevice(i));
    for(int num_batch = 0; num_batch < NUM_BATCH; num_batch++){
      CHECK_CUDA(cudaMemcpyAsync(local_output + i * prompts_per_gpu * output_seq_len + num_batch * batch_size * output_seq_len, gpu_output[i][num_batch],  batch_size * output_seq_len * sizeof(int), cudaMemcpyDeviceToHost, streams_extra[i][num_batch]));
    }
  }
...
// Synchronize CUDA streams
  for(int i = 0; i < ngpu; i++){
    CHECK_CUDA(cudaSetDevice(i));
    for(int num_batch = 0; num_batch < NUM_BATCH; num_batch++){
        CHECK_CUDA(cudaStreamSynchronize(streams_extra[i][num_batch]));
    }
  }
